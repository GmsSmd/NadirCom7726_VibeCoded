	...
	<div class="container" align="center">
			<form action="includes/getmfsupload_iq.php" method="post" name="upload_excel" enctype="multipart/form-data">
				<fieldset>
				    ...
				    <lable>Skip First Entries:</lable>
				    <input type="number" name="skpEntries" value="26"></input>
							...
							<tr>
								...
								<td>
									<?php 
										if(!$data['do_name']){
											?>
											<select name="EmpSelect" id="tBoxSpecial" class="customSelect" onchange="myFunction(this, this.value)">
												<option >---</option>
												<?php
													$doQ=mysqli_query($con,"SELECT EmpName from empinfo WHERE EmpStatus='Active' Order by sort_order ASC");
													while($data01=mysqli_fetch_array($doQ))
														{
															echo '<option value="'.$data['ref_id'].'">';
																echo $data01['EmpName'];
															echo '</option>';
														}
												?>
...
<script>
function myFunction(element, val) {
	var selectedText = element.options[element.selectedIndex].text;
	var selectedValue= element.options[element.selectedIndex].value;
	if(selectedText !='' && selectedValue !='')
	{
		var xmlhttp = new XMLHttpRequest();
		xmlhttp.open("GET","edit/updateiqdo.php?refId="+selectedValue+"&doName="+selectedText,true);
		xmlhttp.send();
	}
	location.reload();
}
