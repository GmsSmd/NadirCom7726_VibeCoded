...
<head>
	...
	<div class="container" align="center">	
							<center>
								<caption> <h2>Bank Statement for: <?php echo $query_date; ?></h2></caption>
							</center>
						...
						while($data=mysqli_fetch_array($doQ))
							{
								...
								if($name== $bank)
									...
									$nm='doLink';
								?>
								<a href="bankstatementdowisetoday.php?for=<?php echo $name;?>" id="<?php echo $nm;?>" class="button">
								<?php
								echo $data['Name']."</a>";
							}
					?>
						...
						while($data=mysqli_fetch_array($doQ))
							{
								...
								echo "<tr>";
									...
									echo "</td>";
										...
										$q=mysqli_query($con," SELECT sum(rpAmnt) FROM receiptpayment WHERE rpStatus='ReceivedFrom' AND rpFromTo='$emp' AND rpMode='$bank' AND rpDate ='$dateNow' ");
										$Data=mysqli_fetch_array($q);
										$amntDeposit=$Data['sum(rpAmnt)'];
									echo "<td>".$amntDeposit."</td>";
									
										$q=mysqli_query($con," SELECT sum(rpAmnt) FROM receiptpayment WHERE rpStatus='PaidTo' AND rpFromTo='$emp' AND rpMode='$bank' AND rpDate='$dateNow'");
										$Data=mysqli_fetch_array($q);
