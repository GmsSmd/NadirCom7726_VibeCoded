...
if (isset($_POST['getActivity']))
	{
		...
		{	
			$LoadAmntSum=0;
			$sql = mysqli_query($con,"SELECT * FROM tbl_mobile_load WHERE loadStatus='Received' AND loadEmp= '$parentCompany' AND loadDate BETWEEN '$StartDate' AND '$EndDate' ")or die(mysqli_query());
			WHILE($Data=mysqli_fetch_array($sql))
			{
				$Dat=$Data['loadDate'];
				$Amnt= $Data['loadAmnt'];
		...
		{	
			$mfsAmntSum=0;
			$sql1 = mysqli_query($con,"SELECT * FROM tbl_financial_service WHERE mfsStatus='Received' AND mfsEmp= '$parentCompany' AND mfsDate BETWEEN '$StartDate' AND '$EndDate' ")or die(mysqli_query());
