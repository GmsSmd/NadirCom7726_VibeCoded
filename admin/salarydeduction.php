...
if($Employee=="do" or $Employee=="DO")
...
else
	{
		...
		if (isset($_POST['Savecomission']))
		{
			...
			{
			$sq = mysqli_query($con,"INSErt INTO comission(comDate,rp, comType, comEmp, comAmnt, comComments) values('$dateNow','$CurrentMonth', '$type','$emp' ,$Amnt, '$Cmnt' )")or die(mysqli_query());
			echo '<script type="text/javascript">alert("Deduction Rs. '.$Amnt.' successfully added.");</script>';
			}
									...
									<tr>
										...
										<th>Action</th>
											...
											<tbody>
													...
													$dateNow1=date('Y-m-d');
													$sum=0;
													$sql = mysqli_query($con,"SELECT * FROM comission WHERE rp='$CurrentMonth' AND comType='Salary Deduction' AND comEmp='$Employee'")or die(mysqli_query());
