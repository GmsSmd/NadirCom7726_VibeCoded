...
if (isset($_POST['AddIntoProfit']))
	{
		...
		else
		{
				...
				$valuesToAdd= " 'ProfitAdd','$dt','ReceivedFrom','ProfitAdd',$Amnt, ''";
...
if (isset($_POST['SubFromProfit']))
	{
		...
		{
				$columnsToAdd="rpFor,rpDate,rpStatus, rpFromTo, rpAmnt, rpMode";
				$valuesToAdd= " 'Withdraw','$dt','PaidTo','ProfitSub',$Amnt, '$defaultBankName'";
					//echo "$columnsToAdd". "<br>"."$valuesToAdd";
				$sq = mysqli_query($con,"INSERT INTO receiptpayment($columnsToAdd) values($valuesToAdd)")or die(mysqli_query());
		}
	}
											...
											<form id="" name="" action="" method="POST">
													<table cellpadding="0" cellspacing="0" border="0" class="table" id="HeadTable" >
																...
																<tr>
																	...
																	<td style="text-align: right;" >Add Into Profit: 
																	</td>
																...
																<tr>
																	...
																	<td style="text-align: right;" >Take From Profit: 
																	</td>
										...
										<th>Balance</th>
											...
											<?php
													...
													$Opening=$Data5['ocAmnt'];
														...
														echo "</td>";
														echo "<td>".$Opening."</td>";
															$q=mysqli_query($con,"SELECT sum(rpAmnt),rpNotes FROM receiptpayment WHERE rpFor='ProfitAdd' AND rpDate ='$cd' AND rpStatus='ReceivedFrom' AND rpFromTo='ProfitAdd' ");
															$Data=mysqli_fetch_array($q);
															$amntPaid=$Data['sum(rpAmnt)'];
														echo "<td>".$amntPaid."</td>";
														echo "<td>".$Data['rpNotes']."</td>";
														
															$q=mysqli_query($con,"SELECT sum(rpAmnt),rpNotes FROM receiptpayment WHERE rpFor='Withdraw' AND rpDate ='$cd' AND rpStatus='PaidTo' AND rpFromTo='ProfitSub'");
															$Data=mysqli_fetch_array($q);
